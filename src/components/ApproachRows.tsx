...
export default function ApproachRows() {
  ...
  return (
    <section className="px-5 pt-8 pb-10 sm:px-8 sm:pt-10 sm:pb-12 lg:px-12 lg:pt-10 lg:pb-12" style={{ backgroundColor: "#fff" }}>
            ...
            return (
              <div key={row.id}>
                <button
                  ...
                  className="group flex w-full items-baseline justify-between gap-5 py-[10px] pr-[60px] text-left transition-colors duration-150 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#1a7a47] sm:py-[17px] sm:pr-[76px]"
                ...
                >
                  ...
                  <span aria-hidden="true" className="self-center transition-transform duration-200 group-hover:translate-x-1" style={{ fontFamily: "var(--font-body)", color: GREEN, fontSize: "1.15rem" }}>
                    {isActive ? "−" : "+"}
                  </span>
                </button>

                {isActive && <Spread item={row} onClose={handleClose} />}
              </div>
            )
          })}
        </div>
    ...
    >
      <div className="flex flex-col px-6 py-9 sm:px-9 sm:py-11 lg:px-12 lg:py-14" style={{ order: isLearn ? 2 : 1 }}>
        <div className="flex flex-1 flex-col justify-center">
          <div className="flex items-baseline gap-4" style={{ margin: "0 0 0.9rem" }}>
            <span style={{ fontFamily: "var(--font-body)", fontSize: "0.78rem", fontWeight: "var(--font-weight-medium)", color: GREEN }}>
              {item.number}
            </span>
            <h2 style={{ fontFamily: "var(--font-holiday-display), Impact, sans-serif", fontWeight: 400, fontSize: "clamp(3.6rem, 9vw, 8.5rem)", lineHeight: 0.82, letterSpacing: "-0.03em", color: "var(--color-black)", margin: 0, textTransform: "uppercase" }}>
              {item.word}.
            </h2>
          </div>
