      ...
      <div style={{ backgroundColor: "#0f2b1c", color: "var(--color-white)" }}>
        <div className="mx-auto px-5 py-8 sm:px-8 sm:py-9 lg:px-12 lg:py-10" style={{ maxWidth: "1320px" }}>
          <div className="grid grid-cols-1 divide-y divide-white/20 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
            {SCHOOLS_TRUSTED_PARTNERS.stats.map((stat) => (
              <div key={stat.primary} className="flex flex-col items-center justify-center px-4 py-6 text-center sm:px-6 sm:py-7">
                <p className="flex items-center justify-center" style={{ height: "clamp(3.45rem, 4.6vw, 4.7rem)", fontFamily: "var(--font-holiday-display), Impact, sans-serif", fontWeight: 400, fontSize: stat.primary === "ACROSS THE UAE" ? "clamp(2.15rem, 3.15vw, 3.5rem)" : "clamp(3.15rem, 4.5vw, 4.65rem)", lineHeight: 0.9, letterSpacing: "-0.025em", color: "var(--color-white)", margin: 0 }}>
                  {stat.primary}
                </p>
          ...
          <div className="mt-6 border-t border-white/20 pt-6 sm:mt-7 sm:pt-7">
            ...
            <div className="relative mx-auto mt-6 w-full sm:mt-7" style={{ maxWidth: "1100px", aspectRatio: "1809 / 112" }}>
              <Image
                src={SCHOOLS_TRUSTED_PARTNERS.logosImage}
                alt={SCHOOLS_TRUSTED_PARTNERS.logosImageAlt}
                fill
