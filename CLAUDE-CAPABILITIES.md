# Young Icons — Agents & Orchestrator Capabilities (Claude-side)

Referenced from CLAUDE.md. Load this when a task involves picking an agent/subagent, or when drafting a prompt that might benefit from a specific ambient skill. This is Claude Code / Cowork orchestrator context only — Codex reads `AGENTS.md` instead, and none of this file is relevant to it.
...
- **codex** (`codex@openai-codex`, installed 2026-07-30) — spawns the real Codex CLI as a separate sandboxed reviewer agent (`/codex:review`, `/codex:adversarial-review`). Own model, own sandbox, doesn't touch Claude tools/skills or this repo's Claude-side context; billed to the user's ChatGPT/Codex subscription, not Claude tokens. Reads `AGENTS.md` (repo root, hierarchical) automatically on entry — never this file. Standing use: required review pass before any merge from `young-icons-v2` into `master` (see RULES.md). Review-gate-on-every-turn was evaluated and deliberately left off — too much friction/subscription usage for trivial diffs.
